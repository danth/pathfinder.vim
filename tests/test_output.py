from pathfinder.output import get_count, compact_motions, explained_motions
...
def test_explained_motions():
    motion1 = Motion({"motion": "j".encode(), "weight": 1, "description": "Down {count} lines".encode()})
    motion2 = Motion({"motion": "k".encode(), "weight": 1, "description": "Up {count} lines".encode()})
    assert (
        list(explained_motions([motion1, motion2, motion2]))
        == ["j    Down 1 lines", "2k   Up 2 lines"]
    )
