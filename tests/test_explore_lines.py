...
from pathfinder.explore_lines import get_explore_lines, get_line_limits


def test_get_explore_lines():
    with mock.patch("pathfinder.explore_lines.vim.vars",
...
@mock.patch("pathfinder.explore_lines.get_explore_lines", return_value=0)
def test_get_line_limits(mock_get_explore_lines):
    with mock.patch("pathfinder.explore_lines.vim.current.buffer", ["line"] * 10):
        assert get_line_limits({"lnum": 2}, {"lnum": 8}) == (2, 8)
        assert mock_get_explore_lines.called_once_with(6)

@mock.patch("pathfinder.explore_lines.get_explore_lines", return_value=2)
